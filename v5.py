...
def create_weekly_keyboard(available_slots):
    ...
    for day_slots in available_slots:
        ...
        for slot_info in day_slots:
            ...
            callback_data = f"reserve_{slot_info['day']}_{slot_info['start_minutes']}_{slot_info['end_minutes']}"
...
def reserve_slot(message):
    ...
    selected_day = selected_text.split('(')[0].strip()
    selected_time = selected_text.split('(')[1].split(')')[0].strip()
